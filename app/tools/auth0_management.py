...
async def list_user_organizations(user_id: str) -> list[dict]:
    ...
    Used by the top-nav switcher so multi-org users (e.g. an agent
    shared between two travel agencies) can see and hop between
    their orgs instead of being pinned to whichever one Auth0
    selected at initial login.
    """
    token = await _get_management_token()
    async with httpx.AsyncClient(timeout=15.0) as client:
        resp = await client.get(
