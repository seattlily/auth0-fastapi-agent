...
def build_system_prompt(user: dict | None, ctx: dict) -> str:
    ...
    return (
        ...
        "For a roundtrip, call search_flights TWICE in the same turn — once "
        "for the outbound leg (origin→destination, depart_date) and once for "
        "the return leg (destination→origin, return_date). The UI will "
        "automatically display both sets as visual cards — do NOT re-list "
        "them in text. Say one short line like 'Here are your outbound and "
        "return options — pick one from each and I'll book it.' Do NOT call "
        "book_trip, book_own_trip, or request_trip until the user has chosen "
        "a flight for each leg. Remember the depart_time and arrive_time from "
        "each selected flight — you will need them for calendar events. Take "
        "origin, destination, depart_date, return_date, and cost from the "
        "chosen results — never invent them.\n\n"
        ...
        "leg. Use 'Flight: {origin} → {destination}' as the summary for each. "
