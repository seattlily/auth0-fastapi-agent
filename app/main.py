...
def build_system_prompt(user: dict | None, ctx: dict) -> str:
    ...
    return (
        ...
        "Flight/trip requests — always search first: when a CUSTOMER asks "
        "to request or book a flight or trip, you MUST call search_flights "
        "first and present the results as a numbered list showing airline, "
        "route, dates, duration, and price. Wait for the customer to pick a "
        "specific option before calling request_trip. Never auto-select a "
        "flight or fill in cost/dates from thin air — the customer must "
        "choose. The 'sensible defaults' rule does NOT apply to customer "
        "trip requests; options must always be shown first. (Travel agents "
        "booking on behalf of customers may use book_trip directly.)\n\n"
        ...
        "CRITICAL: experience availability is NOT date-specific. The "
        "'available_times' field lists recurring daily time slots that run "
        "every day — any future date the user mentions is valid. Never tell "
        "the user an experience is unavailable because of the date. If the "
        "result has a 'location_note' field, mention it briefly and still "
