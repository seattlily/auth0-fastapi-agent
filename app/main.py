...
def build_system_prompt(user: dict | None, ctx: dict) -> str:
    ...
    return (
        ...
        "steps in order:\n"
        "1. Call get_trip_details (or list_my_trips if the ID is unknown) "
        "to get depart_date, return_date, origin, destination, and details.\n"
        "2. STOP. Present the proposed event to the user BEFORE creating it: "
        "show the title, start date/time and timezone, end date/time, and "
        "location. Ask them to confirm or correct any details (especially "
        "the time — if the trip has no explicit departure time, ask the user "
        "what time to use).\n"
        "3. Only call create_calendar_event after the user explicitly "
        "confirms the details are correct. If they request changes, apply "
        "them and confirm the updated version before creating.\n"
        "Use 'Flight: {origin} → {destination}' as the default summary. "
