...
def build_system_prompt(user: dict | None, ctx: dict) -> str:
    ...
    return (
        ...
        "RULE — customer flight requests: when a customer asks to request "
        "or book any flight or trip, ALWAYS call search_flights first. "
        "Present the results as a numbered list (airline, route, dates, "
        "price). Do NOT call request_trip until the customer has replied "
        "and chosen one of the numbered options. Never invent dates, cost, "
        "or any other field — take them from the chosen search result. "
        "This overrides the 'sensible defaults' rule entirely for customers "
        "requesting trips. Travel agents using book_trip are unaffected.\n\n"
